 ...
 * @package       Cake.Model.Datasource.Database
 */
class SqlserverDblib extends DboSource {
 ...
 * @var string
 */
	public $description = "SQL Server PDO Driver (Sybase)";
        ...
        */
		...
		try {
			$this->_connection = new PDO(
				"dblib:host={$config['host']};dbname={$config['database']}",
				$config['login'],
				$config['password']
			);
			$this->connected = true;
 ...
 */
		...
		if ($count >= 1 && strpos($fields[0], 'COUNT(*)') === false) {
			...
			for ($i = 0; $i < $count; $i++) {
				...
				if (!preg_match('/\s+AS\s+/i', $fields[$i])) {
                    ...
                    // PHP BUG: Column names truncated at 29 chars
                    // http://stackoverflow.com/questions/4658510/sql-server-returned-column-name-truncated-to-29-characters-using-php
                    $maxLen = 29;
                    $maxLenCol = $maxLen - strlen($alias) - 2;

					if (strpos($fields[$i], '.') === false) {
                        //$this->_fieldMappings[$alias . '__' . $fields[$i]] = $alias . '.' . $fields[$i];
                        $field = $fields[$i];
                        if (strlen($field) > $maxLenCol) {
                            $field = substr(uniqid(), 0, $maxLenCol);
                        }
                        $field = $this->_mapField($alias, $fields[$i]);

                        $mapAlias = $alias . '__' . $field;
                        $mapField = $alias . '.' . $fields[$i];
						$fieldName = $this->name($mapField);
						$fieldAlias = $this->name($mapAlias);
					} else {
						$build = explode('.', $fields[$i]);
                        ...
                        $field = $this->_mapField($alias, $build[1]);
						$name = $build[0] . '.' . $build[1];
						$alias = $build[0] . '__' . $field;
                    ...
                    $this->_fieldMappings[$mapAlias] = $mapField;
    ...
    protected function _mapField($tableAlias, $field) {
        // PHP BUG: Column names truncated at 29 chars
        // http://stackoverflow.com/questions/4658510/sql-server-returned-column-name-truncated-to-29-characters-using-php
        $maxLen = 29;
        $maxLenCol = $maxLen - strlen($tableAlias) - 2;

        if (strlen($field) <= $maxLenCol) {
            return $field;
        }

        $part = substr($field, 0, $maxLenCol - 2);
        $i = 1;
        do {
            $field = $part . '_' . $i++;

        } while (isset($this->_fieldMappings[$tableAlias . '__' . $field]));
		...
		switch (strtolower($type)) {
			case 'select':
                ...
                if (version_compare($this->getVersion(), '11', '<') && preg_match('/FETCH\sFIRST\s+([0-9]+)/i', $limit, $offset)) {
                    ...
                    // Folder SQL Server 2000 use the exclude-subquery version of pagination **MSSQL suxx**
                    if (version_compare($this->getVersion(), '9', '<')) {

                        $colUnique = "[NUMMER]";
                        $sql = "SELECT {$limit} {$fields} FROM {$table} {$alias}
                                  WHERE {$colUnique} NOT IN (
                                    SELECT TOP {$offset} {$colUnique}
                                    FROM {$table}
